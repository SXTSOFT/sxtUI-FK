...
(function () {
  'use strict';
  localPack.$inject = ["db", "$http", "$rootScope", "$cordovaFileTransfer", "sxt", "xhUtils"];
  angular
    .module('app.xhsc')
    .factory('localPack', localPack);
  /** @ngInject */
  function localPack(db,$http,$rootScope,$cordovaFileTransfer,sxt,xhUtils) {
...
/*            gx.forEach(function (g) {
            ...
            areas.forEach(function (area) {
              ...
              pics.forEach(function (u) {
                config.tasks.push({
                  ...
                  url:'http://ggem.sxtsoft.com:9191/Api/Picture/Tile/'+u+'?path='+area.DrawingImageUrl
                });
              })
            });
            $rootScope.$emit('pack'+self.config._id,{
        ...
        if(task.type == 'file') {
          try {
            var rootPath = cordova.file.documentsDirectory +'/'+ task._id,
