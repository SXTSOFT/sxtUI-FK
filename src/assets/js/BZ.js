...
L.Draw.Feature = L.Handler.extend({
	...
	disable: function (fireDisabled) {
		if (!this._enabled) { return; }

		L.Handler.prototype.disable.call(this);

		this._map.fire('draw:drawstop', { layerType: this.type });
		if(fireDisabled!==false)
...
/*					case 3:
	...
	L.Draw.Stamp = L.Draw.SimpleShape.extend({
		...
		_clickToCreate:function(e) {
			if(this._map && this._map._popup && this._map._popup._container.contains(e.target)){
				return;
			};
			if(e.changedTouches){
				e.clientX = e.changedTouches[0].clientX;
	...
	L.Stamp.include(!L.Path.CANVAS ? {} : {
		...
		projectLatlngs:function() {
			if (this._map) {
				this._point = this._map.latLngToLayerPoint(this._latlng);
				var radius, fontSize, strokeWidth, zoom = this._map.getZoom();
				radius = Math.pow(2,1+Math.min(Math.max(2,this._map._zoom),6));
				fontSize = radius*1.5;
