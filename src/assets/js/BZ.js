...
/*					case 3:
	...
	L.Draw.Stamp = L.Draw.SimpleShape.extend({
		...
		_clickToCreate:function(e) {
			if(this._map && this._map._popup){
				L.Draw.Stamp.isInPop = true;
				if(!this._map._popup._container.contains(e.target)) {
					this._map.closePopup();
					L.Draw.Stamp.isInPop = false;
					return;
				}
			};
			if(L.Draw.Stamp.isInPop){
				L.Draw.Stamp.isInPop = false;
				return;
			}
			L.Draw.Stamp.isInPop = true;
...
/*			var bounds = this._shape.getBounds(),
			...
			this._moveMarker = this._createMarker(this._shape._latlng, new L.DivIcon({
				iconSize: new L.Point(this._shape._fontSize*1.5, this._shape._fontSize*1.5),
				className: 'leaflet-div-icon leaflet-editing-icon leaflet-edit-move'
			}));
	...
	L.Stamp.include(!L.Path.CANVAS ? {} : {
		...
		projectLatlngs:function() {
			if (this._map) {
				this._point = this._map.latLngToLayerPoint(this._latlng);
				var radius, fontSize, strokeWidth, zoom = this._map.getZoom();
				radius = Math.pow(2,1+Math.min(Math.max(2,this._map._zoom),3));
				fontSize = radius*1.5;
		...
		_drawPath: function () {
			var e = this.getPathString();
			this._ctx.beginPath();
			this._ctx.arc(e.x, e.y, e.radius, 0, Math.PI * 2, false);
			this._ctx.font = this._fontSize+'px serif';
			this._ctx.textAlign = 'center';
			this._ctx.fillStyle = this._getColor();
			this._ctx.fillText((e.str||this.string), e.x, e.y+(e.radius/2.5), e.radius);
		},
		_updatePath:function () {
			L.Path.prototype._updatePath.call(this);
		},
	...
	L.SvFeatureGroup = L.FeatureGroup.extend({
		...
		_onClick:function(e){
			...
			this.eachLayer(function (layer) {
				...
				if(bounds && L.SvFeatureGroup.isMiddleNumber(bounds._southWest.lat, bounds._northEast.lat, p.lat, 0) &&
					L.SvFeatureGroup.isMiddleNumber(bounds._southWest.lng, bounds._northEast.lng, p.lng, 0)
				){
	...
	L.Svgline = L.Polyline.extend({
		...
		_drawPath: function () {
			...
			/*if(this.v.type == 'A'){
			 ...
			 this._ctx.beginPath();
			 if(this.v.ex.indexOf('0 ')!=-1) {
			 var p1 = parts[0][0],
			 p2 = parts[0][1],
			 r = Math.abs(p1.x - p2.x),
			 a = p2.x>p1.x && p2.y<p1.y?0:
			 p2.x<p1.x && p2.y>p1.y?1:
			 p2.x<p1.x && p2.y<p2.y?2:3,
			 f = a*Math.PI/2,
			 t = f+Math.PI/2;
			 this._ctx.arc(p2.x, p1.y, r, f, t);
			 }
			 else {
			 var p1 = parts[0][0],
			 p2 = parts[0][1],
			 ...
			 f = a*Math.PI/2,
			 t = f+Math.PI/2;
			 this._ctx.arc(p1.x, p2.y, r, f, t);
			 }
			 //this._ctx.moveTo(parts[0][0].x, parts[0][0].y);
			 //this._ctx.lineTo(parts[0][1].x, parts[0][1].y);
			 }
			 }
			 else{*/
			return L.Path.prototype._drawPath.call(this);
			//}
		}
	});
	L.Svgline.include(!L.Path.CANVAS ? {} : {
		_updateStyle:function () {
			for (var k in this.v.attrs) {
				//if (this.v.attrs.hasOwnProperty(k))
				switch (k) {
					case 'stroke-width':
						this._ctx.lineWidth = this.v.attrs[k];
						break;
					case 'stroke':
						this._ctx.strokeStyle = this.v.attrs[k];
						break;
