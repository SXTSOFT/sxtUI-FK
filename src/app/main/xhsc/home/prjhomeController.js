...
(function () {
  ...
  angular
    .module('app.xhsc')
    .controller('prjhomeController', prjhomeController);

  /** @ngInject */
  function prjhomeController($scope, $stateParams, $state, $rootScope,api,$mdDialog,remote) {
    var vm = this;
    vm.title = $stateParams.companyName;
    ...
    vm.go = function (item) {
      $state.go('app.progress', {projectId: item.ID, projectName: item.Name});
    }

    function load() {
      remote.progress.getProjectData($stateParams.companyId).then(function (r) {
